        ...
        public static void Callback()
        {
            ...
            switch (cmd)
            {
                ...
                case "spawnfrompool":
                    ...
                    var pool = GameObject.FindObjectsOfType<Pool>().FirstOrDefault(p => p.name.ToLower() == data.ToLower());
                    if (pool == null)
                    {
                        Chaos.Warn("admin sent incorrect pool name " + data);
                        return;
                    }
                    var poolee = pool.InstantiatePoolee(Vector3.one, Quaternion.identity);
