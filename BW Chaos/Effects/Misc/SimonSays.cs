...
namespace BWChaos.Effects
{
    ...
    internal class SimonSays : EffectBase
    {
        ...
        Dictionary<SimonSaysType, bool> conditions = new Dictionary<SimonSaysType, bool>();
        ...
        {
            conditions.Clear();
            foreach (var val in Enum.GetValues(typeof(SimonSaysType)))
            {
        ...
        public IEnumerator CoRun()
        {
            ...
            while (Active)
            {
                var pair = kv.Random();
                var sst = pair.Key;
                var mName = Utilities.GetReadableStringFromMemberName(pair.Value);
                bool didSimonSay = Random.value > 0.5f;
                Vector3 startPos = GlobalVariables.Player_PhysBody.transform.position;

                var nDat = Notifications.SendNotification((didSimonSay ? "Simon says " : "") + mName, 5);
...
#if DEBUG
                        Chaos.Log("Distance between head and feet: " + dist);
#endif
                        break;
...
#if DEBUG
                Chaos.Log($"Did the player {sst}? {conditions[sst]}. Punish? {doPunish}");
#endif
