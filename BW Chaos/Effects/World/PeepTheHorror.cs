...
namespace BWChaos.Effects
{
    internal class PeepTheHorror : EffectBase
    {
        ...
        void Init()
        {
            ...
            var tex = GlobalVariables.EffectResources.LoadAsset<RenderTexture>("Assets/MaterialTextures/PeepTheHorror.renderTexture");
        ...
        public override void OnEffectStart()
        {
            ...
            foreach (var source in Utilities.FindAll<AudioSource>())
            {
        ...
        public override void OnEffectEnd()
        {
            ...
            foreach (var kv in originalMaterials) kv.Key.material = kv.Value;
            foreach (var kv in originalMaterialsSkinned) kv.Key.material = kv.Value;
            foreach (var player in players) player.gameObject.Destroy();
        }
        ...
        IEnumerator ProgressClips()
        {
            ...
            {
#if DEBUG
                Chaos.Log($"Moving to next clip at idx {i} - {clips[i].name}");
#endif
...
#if DEBUG
                    Chaos.Log($"Starting {player.gameObject.name} at {Time.realtimeSinceStartup - clipStartTime} ({Time.realtimeSinceStartup} - {clipStartTime})");
#endif
                    player.Play();
                    yield return new WaitForSecondsRealtime(0.125f);
                    player.volume = 0.02f;
...
#if DEBUG
                Chaos.Log($"Waiting for {timeBetweenClips - players.Length * 0.5f} seconds");
#endif
