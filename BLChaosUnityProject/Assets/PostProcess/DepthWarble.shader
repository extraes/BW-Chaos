Shader "extraes/DepthWarble"
{
    Properties
    {
        [MainTexture] _MainTex("_MainTex", any) = "" {}
        _DepthTex("_DepthTex", any) = "" {}
        _DepthPow("_DepthPow", Range(-1, 10)) = 5
        _DepthLog("_DepthLog", Float) = 2
        [ToggleUI] _OneMinusDepth("_OneMinusDepth", Float) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                depth = depth ? log(pow(depth, _DepthPow)) / log(_DepthLog) : depth;

                half4 albedo = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearMirror_MainTex, IN.uv - float2(0, depth));
