...
{
    static InaccurateGuns()
    {
        GameCallbacks.OnProjectileEnable += SetRecentProjectile;
    }
    ...
    private static void SetRecentProjectile(Projectile __instance)
    {
        recentProjectile = __instance;
    }

#if DEBUG
    internal override async Task<TestResult> Test()
    {
        Poolee poolee = await Barcodes.SpawnAsync(JevilBarcode.MP5, Vector3.zero, Quaternion.identity);
