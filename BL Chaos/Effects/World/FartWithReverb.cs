...
internal class FartWithReverb : EffectBase
{
    ...
    [RangePreference(0.25f, 10, 0.25f)] static readonly float forceMultiplier = 2f;
    ...
    public override void OnEffectStart()
    {
        ...
        for (int i = 0; i < SystemInfo.processorCount / 2; i++)
        {
            ...
            List<Rigidbody> rbPart = rbs.GetRange(min, perCPUCount);
            MelonCoroutines.Start(ApplyForces(rbPart));
#if DEBUG
            Log($"Started {nameof(ApplyForces)} with {perCPUCount} rigidbodies starting at idx {min} idxs=({min}-{min + perCPUCount}) because there are {SystemInfo.processorCount} CPUs");
#endif
        }
#if DEBUG
        sw.Stop();
